...
export function measurePerformance<T>(
  ...
  try {
    ...
    const duration = performance.now() - start;
    logger.error(LogCategory.PERFORMANCE, `Performance measurement failed for ${operation}`, {
      duration: `${duration}ms`,
      ...metadata
    }, error as Error);
    throw error;
...
export async function measurePerformanceAsync<T>(
  ...
  try {
    ...
    const duration = performance.now() - start;
    logger.error(LogCategory.PERFORMANCE, `Async performance measurement failed for ${operation}`, {
      duration: `${duration}ms`,
      ...metadata
    }, error as Error);
    throw error;
