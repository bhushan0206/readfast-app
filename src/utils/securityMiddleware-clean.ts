...
import { logger, LogCategory } from './enhancedLogger';
...
export const AuthSecurity = {
  ...
  validateLoginAttempt: (email: string, password: string): { isValid: boolean; errors: string[] } => {
    ...
    // Log login attempt
    logger.info(LogCategory.SECURITY, 'Login attempt', { email: SecurityUtils.sanitizeInput(email) });
  ...
  validateRegistration: (email: string, password: string, confirmPassword: string): { isValid: boolean; errors: string[] } => {
    ...
    logger.info(LogCategory.SECURITY, 'Registration attempt', { email: SecurityUtils.sanitizeInput(email) });
...
export class SecurityMiddleware {
  ...
  validateFileUpload(file: File): { isValid: boolean; errors: string[] } {
    return ContentSecurity.validateFileUpload(file);
  }
